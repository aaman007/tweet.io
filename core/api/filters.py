...
class OrderingFilter(BaseOrderingFilter):
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        """
        For ordering based on a field that has duplicates can result in ambiguous queryset.
        ex: queryset.order_by('-rating') what if two rows have same rating?
        To solve this ambiguity we append 'id' to the list if it does not exists already,
